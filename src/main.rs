...
async fn main() -> Result<(), Error>{
    ...
    let meta_handle = tokio::spawn(async move{
        if let Err(e) = meta::main(tx_clone, rtx_clone).await{
    ...
    let disk_handle = tokio::spawn(async move{
        if let Err(e) = disks::main(tx_clone, rtx_clone).await{
    ...
    let mem_handle = tokio::spawn(async move{
        if let Err(e) = memory::main(tx_clone, rtx_clone).await{
    ...
    let net_handle = tokio::spawn(async move{
        if let Err(e) = networks::main(tx_clone, rtx_clone).await{
    ...
    let socket_handle = tokio::spawn(async move{
        if let Err(e) = sockets::main(tx_clone).await{
    ...
    let proc_handle = tokio::spawn(async move{
        if let Err(e) = processes::main(tx_clone, rtx_clone).await{
